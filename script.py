...
def compare_driver_version(v1, v2):
    ...
    for token_index in range(max_len):
        if optional_index(v1_tokens, token_index) < optional_index(v2_tokens, token_index):
            return -1
        elif optional_index(v1_tokens, token_index) > optional_index(v2_tokens, token_index):
            return 1
...
def get_newest_remote_version(local_version):
    response = requests.get(f'{DIR_URL}/edgewebdriver?delimiter=%2F&maxresults=9999999&restype=container&comp=list&_=1690564408683&timeout=60000')
    r_dict = xmltodict.parse(response.content)
    remote_versions = list(map(lambda v: v['Name'][:-1] ,r_dict['EnumerationResults']['Blobs']['BlobPrefix']))
...
def download_driver(version):
    ...
    response = requests.get(newest_url, params={
        'stream': True
    })
...
if __name__ == "__main__":
    ...
    if version_compare < 0:
        logging.info(f'local versions {local_newest} is newer that remote {remote_newest}, aborting')
